...
export default function AnalysisSidebar({
  ...
  // Scroll highlighted clause into view
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="flex-1 overflow-y-auto p-4 space-y-4" ref={clauseListRef}>
        ...
        <div className="card p-4 animate-fadeIn">
          ...
          {/* Risk bar */}
          <div className="flex rounded-full overflow-hidden h-2 mb-3" style={{ background: 'rgb(var(--color-border))' }}>
            {stats.total > 0 && (
              <>
          ...
          <div className="flex gap-4 text-xs">
            <button onClick={() => setFilter(f => f === 'Red' ? 'all' : 'Red')} className={`flex items-center gap-1.5 transition-opacity ${filter !== 'all' && filter !== 'Red' ? 'opacity-40' : ''}`}>
              <span className="w-2 h-2 rounded-full" style={{ background: 'rgb(var(--color-risk-high))' }} />
              <span style={{ color: 'rgb(var(--color-text-secondary))' }}>{stats.high} High</span>
            </button>
            <button onClick={() => setFilter(f => f === 'Yellow' ? 'all' : 'Yellow')} className={`flex items-center gap-1.5 transition-opacity ${filter !== 'all' && filter !== 'Yellow' ? 'opacity-40' : ''}`}>
              <span className="w-2 h-2 rounded-full" style={{ background: 'rgb(var(--color-risk-medium))' }} />
              <span style={{ color: 'rgb(var(--color-text-secondary))' }}>{stats.medium} Medium</span>
            </button>
            <button onClick={() => setFilter(f => f === 'Green' ? 'all' : 'Green')} className={`flex items-center gap-1.5 transition-opacity ${filter !== 'all' && filter !== 'Green' ? 'opacity-40' : ''}`}>
              <span className="w-2 h-2 rounded-full" style={{ background: 'rgb(var(--color-risk-low))' }} />
        ...
        {/* Executive Summary */}
        {documentSummary && (
          <div className="card animate-fadeIn" style={{ animationDelay: '.05s' }}>
            <button
            ...
            {expandedSection === 'summary' && (
              <div className="px-4 pb-4 border-t" style={{ borderColor: 'rgb(var(--color-border-light))' }}>
...
function ClauseCard({ clause, isHighlighted, onClick, onAsk, index }: ClauseCardProps) {
  ...
  const riskLabel = clause.category === 'Red' ? '🔴 High Risk' : clause.category === 'Yellow' ? '🟡 Medium Risk' : '🟢 Low Risk'

  return (
    <div
      id={`sidebar-${clause.id}`}
      className={`card cursor-pointer transition-all ${isHighlighted ? 'card-active' : ''}`}
    ...
    >
      ...
      {/* Expand toggle */}
      <div className="px-3 pb-2 flex items-center gap-1 border-t" style={{ borderColor: 'rgb(var(--color-border-light))' }}>
        <button
      ...
      {/* Expanded content */}
      {expanded && (
        <div className="px-3 pb-3 border-t animate-fadeIn" style={{ borderColor: 'rgb(var(--color-border-light))' }}>
